      ...
      success: function(data) {
        if($promo_type=='min_promo'){
          if(data.org_cost > data.member_cost) {
            ...
            $('.minprice').html((data.disc_cost_per_night).toFixed(0));
            $('.minorgcost').html((data.org_cost_per_night).toFixed(0));
          } else {
            $('.pricespan').css('display','none');
            $('.minprice').html((data.org_cost_per_night).toFixed(0));
          }
        } else if($promo_type=='itinerary_promo'){
          $('.grand-total').html((parseFloat(data.member_cost)+parseFloat(data.taxes)).toFixed(0));
          $('.total_disc_div').html((data.total_discount).toFixed(0));

          if(data.org_cost > data.member_cost) {
            $('.total_disc_tr').css('display','table-row');
            $('.total_cost_div').html((data.member_cost).toFixed(0));
          } else {
            $('.total_disc_tr').css('display','none');
            $('.total_cost_div').html((data.org_cost).toFixed(0));
          }
        } else {
          if($promo_type == 'min_promo') {
            if(data.org_cost > data.member_cost) {
              ...
              $('.pricespan').css('display','none');
              $('.minprice').html((data.org_cost).toFixed(0));
            }
          }
          ...
          _parent.find('.disc_cost_div').html((data.member_cost).toFixed(0));
          ...
          // console.log(total2);
          $('.total-section #grand-total').html(total2.toFixed(0));
        }
      }
  });
}
