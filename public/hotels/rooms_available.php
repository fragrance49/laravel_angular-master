...
<?php
  ...
  $test=0;
  $room_list=array();
  foreach ($room_info as $key=>$va) {
      $price_nights = 1; //Price based on nights? No=1Yes=total_nights
      $nights= $va->nights;
      $this->load->module('home');
      $discount_return = $this->home->priceChangeOnLogin($va->search_id,$price_nights);
      //echo '<pre>';print_r($discount_return);exit;
      $discount_badge = $discount_return['discount_badge'];
      $disc_msg = $discount_return['disc_msg'];
      $org_cost = $discount_return['org_cost']*$nights;
      $disc_cost = $discount_return['disc_cost']*$nights;
      $member_cost = $discount_return['member_cost']*$nights;
      $org_price_div = $discount_return['org_price_div'];
      $room_list[$member_cost.' '.$key]=$va;

  }
  //krsort($room_list);
  //ksort($room_list);
  ksort($room_list, SORT_NUMERIC);
//echo '<pre>';print_r($room_list);exit;

  foreach ($room_list as $key=> $vale) {
    if($vale->room_runno == $t) {
      $hotel_code= $vale->hotel_code;
      $nights= $vale->nights;
     ...
     //echo  $nights;
      ...
      $org_cost = $discount_return['org_cost']*$nights;
      $disc_cost = $discount_return['disc_cost']*$nights;
      $member_cost_total = $discount_return['member_cost']*$nights;
      $member_cost = $discount_return['member_cost'];
      $org_price_div = $discount_return['org_price_div'];
      // $total_discount = $discount_return['discount'];
      // $promo_id = $discount_return['promo_id'];
      $available_status = $discount_return['available_status'];
      if($member_cost<1){
        $available_status = 'none';
      }
?>
<div class="rooms_loop <?php echo $activeClass ?> <?php echo 'loop'.$test ?>" style="display: <?php echo $available_status; ?>" >
  <div class="row">
    <div class="col-md-2 col-sm-2 col-xs-3">
      <div class="" style="">
        <?php //var_dump($room_gallery_img);?>
        <?php if($imgicon!='') { ?>
        <?php
          $images  = array();
             foreach ($room_gallery_img as $key => $img) {
                      $images[] = get_image_aws($img->gallery_img);
           }

           $out = array_values($images);
           $images_json = json_encode($out);
    ...
    <div class="col-md-3 col-sm-3 col-xs-12 text-right push-top-10 promo_parent_div">
      ...
      <b class="price-tag">
        ...
        <span>
          <span class="price-val disc_cost_div"><?php echo number_format($member_cost,0) ?></span> <sub>USD</sub>
      ...
      <p style="font-size: 11px;color: #333;font-weight: 700;">Per Night</p>
       ...
       <p style="font-size: 12px;color: #333;">
          <i class="fa fa-dollar"></i>
          <span class="price-val-total disc_cost_div"><?php echo number_format($member_cost_total,0) ?></span> <span> For <?php echo $nights.' '. ($nights>1? 'Nights':'Night'); ?></span>
        </p>
  ...
  <div class="col-md-12">
    <div class="total-section">
        <div class="price-details" style="margin: 10px;">
            <button type="Submit" name="reserve" id="reserve_btn" class="btn book-btn" style="background: #005aaa;border-color: #005aaa;">Reserve</button>
        </div>
